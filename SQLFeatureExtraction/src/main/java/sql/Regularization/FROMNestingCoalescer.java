...
/**
 * The module tries to merge sub-queries nested in FROM clause with its parent query
 * and thus flattens the structure of a query.
...
public class FROMNestingCoalescer {
	...
	private static PlainSelect stickTopQueryToBottom(PlainSelect bottom,PlainSelect top){
		...
		if(top.getDistinct()!=null){
			if(target.getDistinct()!=null){
				target.getDistinct().getOnSelectItems().addAll(top.getDistinct().getOnSelectItems());
			}
			else{
		...
		if(top.getLimit()!=null){
			if(target.getLimit()==null)
				...
				//TODO currently very rough implementation
		...
		if(top.getGroupByColumnReferences()!=null){
			if(target.getGroupByColumnReferences()!=null){
				target.getGroupByColumnReferences().addAll(top.getGroupByColumnReferences());
			}
			else
		...
		if(top.getOrderByElements()!=null){
			if(target.getOrderByElements()!=null){
				target.getOrderByElements().addAll(top.getOrderByElements());
			}
			else
		...
		if(from instanceof SubSelect){
			...
			if(body instanceof PlainSelect){
				...
				if(QueryToolBox.ifContainAggregate(pps)){
					...
					//we can safely merge
					if(ps.getJoins()==null){						
						return stickTopQueryToBottom(pps,ps);
					}
				pass=false;
...
//		System.out.println("I am returning "+ps);
	...
	private static SelectBody coalesceSubSelectWithHostQuery(PlainSelect host,Join j){
		...
		for (Join myjoin: sublist){
			...
			//add in from items
			List<Join> subJoinList=new ArrayList<>();
			...
			if(mysub.getJoins()!=null)
				subJoinList.addAll(mysub.getJoins());
			...
			if(!subJoinList.isEmpty()){
				if(mergedhost.getJoins()==null)
					...
					mergedhost.getJoins().addAll(subJoinList);
			}
		...
		if(body instanceof PlainSelect){
			...
			for (PlainSelect ps:plist){
				...
				if(b instanceof PlainSelect)
					...
					newplist.addAll(((Union)b).getPlainSelects());	
			}
			u.setPlainSelects(newplist);
			return u;
		}		
