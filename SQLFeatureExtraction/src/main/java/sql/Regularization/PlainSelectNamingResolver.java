...
/**
 * Sub-Module for SelectBodyNamingResolver that deals with the case that the body is of type PlainSelect
 * @author tingxie
 *
 */
public class PlainSelectNamingResolver{
	...
	public Object searchAliasComplete(String alias,Class[] expectedTypes){
	 ...
	 * @return
	 */
	private Object getAliasedObject(String alias,Class[] expectedTypes){
	...
	private Object getObjectNext(String alias,Class[] expectedTypes){
		Object obj=this.aliasRegister.get(alias);

		if(obj!=null){
			boolean pass=false;
			for(Class c:expectedTypes){				
				if(c.isInstance(obj)){
					pass=true;
					break;
				}
	...
	private Object getObjectDeepest(String alias,Class[] expectedTypes){
	...
	private void replaceAliasInPlainSelect(PlainSelect ps){
		...
		//List<Column> groupbylist=ps.getGroupByColumnReferences();
		if(groupbylist!=null){
			List<Expression> newgroupbylist=new ArrayList<Expression> ();
			//List<Column> newgroupbylist=new ArrayList<Column> ();
			for (Expression exp: groupbylist){
			//for (Column col: groupbylist){
				newgroupbylist.add(replaceAliasedInExpression(exp));
				//newgroupbylist.add((Column)replaceAliasedInExpression(col));
			}
					...
					if(obj!=null){
						...
						//if the alias is pointing to a selectbody
							...
							for (PlainSelectNamingResolver res: rlist){	
								//check if this resolve is itself									
								//try to see if the column name is an alias									
	...
	private Expression findBestMatchedSelectItemFromSelectBody(SelectBody body,Column c){
		if(body instanceof PlainSelect)
			...
			for (PlainSelect ps:plist){
				Expression tempresult=findBestMatchedSelectItemFromPlainSelect((PlainSelect) body,c);
				if(tempresult!=null)
