...
import sql.booleanFormulaEntities.BooleanLiteral;
import sql.booleanFormulaEntities.BooleanNormalClause;
import sql.booleanFormulaEntities.DisjunctiveNormalForm;
import sql.booleanFormulaEntities.FixedOrderExpression;
import sql.booleanFormulaEntities.ToolBox;
 ...
 * 1. X Between A AND B becomes A < X AND X < B
 * 
 * 2. X=CASE When A=B Then C ELSE D..END becomes (A=B AND X=C) OR ( !(A=B) AND X=D).
 * 
 * 3. any Function whose name contains isnull: A=isnull(X,Y) becomes (X IS NULL AND A=X) OR (X IS NOT NULL AND A=Y)
 * 
 * 4. CONVERT(data_type(length),expression,style) Function: A=CONVERT(?,B,?) becomes A=B
 * 
 * 5. X IN {a,b} becomes X=a OR X=b
 * @author tingxie
 *
 */
